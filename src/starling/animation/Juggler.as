...
package starling.animation
{
    ...
    public class Juggler implements IAnimatable
    {
		...
		private var mObjectsTemp:Vector.<IAnimatable>;
		...
		{
			var i:int;
			var n:int = mObjects.length;
			mObjectsTemp = new <IAnimatable>[];
			for(i=0; i<n; i++)
			{
				mObjectsTemp[i] = mObjects[i];
			}
			mObjects = new <IAnimatable>[];
		}
		...
		{
			//var i:int;
			//var n:int = mObjectsTemp.length;
			
			//for(i=0; i<n; i++)
			//{
				//mObjects[i] = mObjectsTemp[i];
			//}
			
			mObjects = mObjects.concat(mObjectsTemp);
		}
